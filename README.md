...
- **Multi-Admin RBAC** — create admins with role: `viewer` (read-only), `creator` (create clients only), `editor` (create+edit, no settings), `admin` / `owner` (full). Per-admin inbound scoping via `inboundIds`.
